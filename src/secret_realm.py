...
class SecretRealm:
    ...
    def fun(self, img_list, n):
        if len(img_list) == 1:
            PyautoguiUtils.click_gui(img_daily_path + img_list[0])
        elif len(img_list) > 1:
            PyautoguiUtils.click_gui2(img_daily_path + img_list[0], img_daily_path + img_list[1])
        time.sleep(2)
        # 点击女神球
        PyautoguiUtils.click_godless_ball()
        # 点击扫荡，按钮默认为蓝色，消耗通行证时为黄色
        PyautoguiUtils.click_gui2(img_daily_path + 'sweep1.png', img_daily_path + 'sweep2.png')
        # 点击确定
